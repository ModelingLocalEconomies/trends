...
for state in states:
    artist_dfs = [pd.read_csv(f"test_csvs/{state}_{artist.replace(' ', '-')}_trend_data.csv") for artist in artists]
    ...
    merged_df.to_csv(f'merged_test_csvs/{state}_trend_data.csv')
